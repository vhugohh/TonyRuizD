...
public class CameraPreviewActivity extends Activity implements Camera.PreviewCallback {
    ...
    facePitchText, horizontalGazeText, verticalGazeText, blinkCounterText, values, values2, values3;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        blinkCounterText = (TextView) findViewById(R.id.blinkCounter);
        ...
        state = (ImageView) findViewById(R.id.states);
    ...
    public void setUI(int numFaces, int smileValue, int leftEyeBlink, int rightEyeBlink, int faceRollValue,
            int faceYawValue, int facePitchValue, PointF gazePointValue, int horizontalGazeAngle, int verticalGazeAngle, int blinkCounter) {
        ...
        blinkCounterText.setText("Blinks: " + blinkCounter);

        if (leftEyeBlink < 50 && rightEyeBlink < 50) {
            Drawable drawable = this.getResources().getDrawable(R.drawable.alert_green);
            state.setImageDrawable(drawable);
            state.setAlpha(204);
        }

        if (leftEyeBlink > 50 && rightEyeBlink > 50 && leftEyeBlink < 80 && rightEyeBlink < 80) {
            Drawable drawable = this.getResources().getDrawable(R.drawable.alert_yellow);
            state.setImageDrawable(drawable);
            state.setAlpha(204);
        }

        if (leftEyeBlink > 81 && rightEyeBlink > 81) {
            Drawable drawable = this.getResources().getDrawable(R.drawable.alert_red);
            state.setImageDrawable(drawable);
            state.setAlpha(204);
        }
    ...
    public void stopCamera() {
        ...
        blinkCounter = prefe.getInt("contador", 0);
    ...
    public void onPreviewFrame(byte[] data, Camera arg1) {
        ...
        if (numFaces == 0) {
            ...
            setUI(0, 0, 0, 0, 0, 0, 0, null, 0, 0, 0);
        } else {
            ...
            if (faceArray == null) {
                ...
                for (int j = 0; j < numFaces; j++) {
                    ...
                    //blinkCounter = faceArray[j].leftEyebrow.top.y - faceArray[j].leftEyebrow.bottom.y;
                    values.setText("LEo.top.y: " + faceArray[j].leftEyeObj.top.y + "LEo.bottom.y: " + faceArray[j].leftEyeObj.bottom.y);
                    values2.setText("Diferencia: " + (faceArray[j].leftEyeObj.bottom.y - faceArray[j].leftEyeObj.top.y));

                    int diferencia  = faceArray[j].leftEyeObj.bottom.y - faceArray[j].leftEyeObj.top.y;
                    SharedPreferences prefe=getSharedPreferences("datos",Context.MODE_PRIVATE);
                    blinkCounter = prefe.getInt("contador", 0);
                    ...
                    Editor editor=prefe.edit();
                    editor.putInt("contador", blinkCounter);
                    editor.commit();
                    values3.setText("Parpadeos: " + blinkCounter);
                    String strMsg = " : " +faceArray[j].getSmileValue() +
                                    " : " +faceArray[j].leftEye.x +
                                    " : " + faceArray[j].leftEye.y +
                                    " : " + faceArray[j].rightEye.x +
                                    " : " + faceArray[j].rightEye.y +
                                    " : " + faceArray[j].leftEyeObj.centerPupil.x +
                                    " : " + faceArray[j].leftEyeObj.centerPupil.y +
                                    "";
                    if (bandera) {
                        MediaPlayer mPlayer = MediaPlayer.create(CameraPreviewActivity.this, R.raw.click);
                        mPlayer.start();
                    }

                    Log.d("VIC", strMsg);
                    /*
                    Log.d("VIC:"  leftEye.x: ",String.valueOf(faceArray[j].leftEye.x));
                    Log.d("VIC: leftEye.y: ",String.valueOf(faceArray[j].leftEye.y));
                    Log.d("VIC: rightEye.x: ",String.valueOf(faceArray[j].rightEye.x));
                    Log.d("VIC: rightEye.y: ",String.valueOf(faceArray[j].rightEye.y));
                    */
                }
                setUI(numFaces, smileValue, leftEyeBlink, rightEyeBlink, faceRollValue, yaw, pitch, gazePointValue,
                        horizontalGaze, verticalGaze, blinkCounter);
            }
        }
    }
